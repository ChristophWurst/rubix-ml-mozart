...
class SwissRoll implements Generator
{
    ...
    public function __construct(
        ...
        $this->center = TEST_Vector::quick([$x, $y, $z]);
        $this->scale = $scale;
        $this->depth = $depth;
        $this->noise = $noise;
    }
    ...
    {
        $t = TEST_ColumnVector::rand($n)
            ->multiply(2)
            ->add(1)
            ->multiply(1.5 * M_PI);

        $x = $t->multiply($t->cos());
        $y = TEST_ColumnVector::rand($n)->multiply($this->depth);
        $z = $t->multiply($t->sin());

        $noise = TEST_Matrix::gaussian($n, 3)
            ->multiply($this->noise);
            
        $samples = TEST_Matrix::stack([$x, $y, $z])
            ->multiply($this->scale)
            ->add($this->center)
            ->add($noise)
            ->asArray();
