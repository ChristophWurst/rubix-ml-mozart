...
class GradientBoost implements Estimator, Learner, RanksFeatures, Verbose, Persistable
{
    ...
    /**
     * @param \Test\Vendor\Rubix\ML\Learner|null $booster
     * @param float $rate
     * @param float $ratio
    ...
    public function train(Dataset $dataset) : void
    {
        ...
        $out = $prevOut = TEST_Vector::quick($this->base->predict($training));
        $target = TEST_Vector::quick($training->labels());

        $prevPred = TEST_Vector::quick($this->base->predict($testing));
        ...
        for ($epoch = 1; $epoch <= $this->estimators; ++$epoch) {
            ...
            $out = TEST_Vector::quick($predictions)
                ->multiply($this->rate)
                ->add($prevOut);
            ...
            $pred = TEST_Vector::quick($predictions)
                ->multiply($this->rate)
                ->add($prevPred);
