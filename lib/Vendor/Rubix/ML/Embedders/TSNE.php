...
use Test\Vendor\Rubix\ML\Specifications\SamplesAreCompatibleWithDistance;
...
class TSNE implements Embedder, Verbose
{
    ...
    public function transform(array &$samples) : void
    {
        ...
        $p = TEST_Matrix::quick($this->affinities($distances))
            ->multiply($this->exaggeration);

        $y = TEST_Matrix::gaussian($m, $this->dimensions)
            ->multiply(self::Y_INIT_SCALE);

        $velocity = TEST_Matrix::zeros($m, $this->dimensions);
        $gains = TEST_Matrix::ones($m, $this->dimensions)->asArray();
        ...
        for ($epoch = 1; $epoch <= $this->epochs; ++$epoch) {
            ...
            $gradient = $this->gradient($p, $y, TEST_Matrix::quick($distances));
            ...
            $gradient = $gradient->multiply(TEST_Matrix::quick($gains));
     ...
     * the embedding.
     *
     * @param \TEST_Tensor\TEST_Matrix $p
     * @param \TEST_Tensor\TEST_Matrix $y
     * @param \TEST_Tensor\TEST_Matrix $distances
     * @return \TEST_Tensor\TEST_Matrix
     */
    protected function gradient(TEST_Matrix $p, TEST_Matrix $y, TEST_Matrix $distances) : TEST_Matrix
    {
